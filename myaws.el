...
(defun aws-print-tags (tags color)
  (let ((tagnum (length tags))
	(sorted-tags (cl-sort tags 'string< :key (apply-partially 'format "%s"))))
    (if (eq tagnum 0)
	(insert (propertize (format "NO-TAGS " tagnum) 'face '(:foreground "red")))
      (progn (insert (propertize " Tags:" 'face '(:foreground color)))
	     (mapcar (lambda (x) (insert (propertize (format " %s/%s" (cdr (assoc 'Key x)) (cdr (assoc 'Value x))) 'face '(:foreground color)))) sorted-tags)))))
...
(defun aws-print-instances-single (element)
  (let*
    ...
    (if (string= State "running")
        (progn
          (aws-create-link-for-instance "teal" PrivateIpAddress (or (cdr (assoc 'Value (find-if (lambda (x) (equal (assoc-default 'Key x) "Name")) tags))) "NoName"))
...
(defun aws-print-image (element)
  (let*
      ((VirtualizationType (cdr (assoc 'VirtualizationType element)))
       ...
       (ImagesId (cdr (assoc 'ImageId element)))
	...
	(progn
	  ...
	  (insert (propertize (format "ID: %s " ImagesId) 'face '(:foreground "green")))
...
(defun aws-create-link-for-instance (color ip name)
  "Insert clickable string inside a buffer"
  ;;(XxawsX: create_link color:%s ip:%s name:%s" color ip name)
  (lexical-let ((color color)
		(map (make-sparse-keymap)))
    (define-key map (kbd "<RET>")
...
(defun dump-images-list ()
  (interactive)
  (async-shell-command "aws ec2 describe-images --owners self > ~/.emacs.d/ec2-describe-images.json" "*dump-images-list*" "*dump-images-error*"))
